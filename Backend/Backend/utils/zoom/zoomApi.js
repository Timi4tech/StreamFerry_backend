...
const os = require('os');
const {ensureZoomAccessToken} = require('./zoomAuth')
dotenv.config();
...
async function fetchZoomRecordings(accessToken) {
  try {
 ...
 */
async function downloadFromZoom(downloadUrl, token, fileName) {
  try {
...
async function getZoomStream(downloadUrl, zoomToken) {
